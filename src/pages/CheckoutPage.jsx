...
import { CreditCard, Banknote, ArrowLeft, ShieldCheck, Gift } from "lucide-react";
...
const CheckoutPage = () => {
  ...
  const [formData, setFormData] = useState({ name: "", email: "", phone: "", address: "", paymentMethod: "stripe", giftMessage: "" });
  ...
  const handleApplyPromo = async () => {
    ...
    catch { setPromoError("Error verifying code"); }
  };
  ...
  const handleInitialFormSubmit = async (e) => {
    ...
    if (formData.paymentMethod === "stripe") {
      setIsLoadingSecret(true); setApiError(null);
      try {
        const response = await fetch("/api/create-payment-intent", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ items: [{ name: "Cart Order", price: totalAmountStr }], isCustom: false }) });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error?.message || "Failed to create payment intent");
        else if (data.error) setApiError(data.error.message || data.error);
        else if (data.clientSecret) setClientSecret(data.clientSecret);
        else throw new Error("Invalid response from payment server");
      } catch (err) { setApiError(err.message.includes("Invalid API Key") ? "Stripe payment temporarily unavailable. Please use Cash on Delivery." : "Could not connect to payment server. Please try Cash on Delivery."); }
      finally { setIsLoadingSecret(false); }
  ...
  return (
    <div className="page-wrapper page-enter-active" style={{ paddingTop: "100px", minHeight: "90vh", background: "#fafafa" }}>
      ...
      <div className="container">
        <Link to="/cart" className="back-link animated-link" style={{ marginBottom: "24px", display: "inline-flex" }}><ArrowLeft size={18} /> Back to Cart</Link>
          ...
          <div className="glass-panel" style={{ padding: "40px", background: "white" }}>
            ...
            {clientSecret ? (
              <div className="stripe-embedded-container reveal-up">
                <button type="button" onClick={() => setClientSecret(null)} style={{ background: "none", border: "none", color: "var(--text-color)", display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", marginBottom: "20px" }}><ArrowLeft size={18} /> Change payment method</button>
                <div style={{ background: "#f8fafc", padding: "24px", borderRadius: "12px", border: "1px solid #e2e8f0" }}>
                  <h3 style={{ fontSize: "1.2rem", marginBottom: "16px" }}>Secure Credit Card Payment</h3>
                  <Elements stripe={stripePromise} options={{ clientSecret }}><StripeCheckoutForm amountStr={totalAmountStr} onSuccess={handleConfirmOrder} onCancel={() => setClientSecret(null)} /></Elements>
                </div>
              </div>
            ) : (
              <form onSubmit={handleInitialFormSubmit} className="checkout-form">
                <div className="checkout-form-grid">
                  <div className="form-group" style={{ marginBottom: "20px" }}><label htmlFor="name" style={{ display: "block", marginBottom: "8px", fontSize: "0.9rem", color: "#555" }}>Full Name</label><input type="text" id="name" name="name" value={formData.name} onChange={handleChange} required placeholder="e.g. Jane Doe" style={{ width: "100%", padding: "12px 16px", border: "1px solid #ddd", borderRadius: "8px", fontSize: "1rem" }} /></div>
                ...
                <div className="form-group" style={{ marginBottom: "32px" }}>
                  ...
                  <div className="checkout-form-grid">
                    <label style={{ display: "flex", alignItems: "center", gap: "12px", padding: "16px", border: formData.paymentMethod === "stripe" ? "2px solid var(--primary-dark)" : "1px solid #ddd", borderRadius: "8px", cursor: "pointer", background: formData.paymentMethod === "stripe" ? "#fafafa" : "#fff" }}><input type="radio" name="paymentMethod" value="stripe" checked={formData.paymentMethod === "stripe"} onChange={handleChange} style={{ accentColor: "var(--primary-dark)", width: "18px", height: "18px" }} /><CreditCard size={20} color={formData.paymentMethod === "stripe" ? "var(--primary-dark)" : "#666"} /><span style={{ fontWeight: formData.paymentMethod === "stripe" ? "600" : "400" }}>Pay Online</span></label>
                    <label style={{ display: "flex", alignItems: "center", gap: "12px", padding: "16px", border: formData.paymentMethod === "cod" ? "2px solid var(--primary-dark)" : "1px solid #ddd", borderRadius: "8px", cursor: "pointer", background: formData.paymentMethod === "cod" ? "#fafafa" : "#fff" }}><input type="radio" name="paymentMethod" value="cod" checked={formData.paymentMethod === "cod"} onChange={handleChange} style={{ accentColor: "var(--primary-dark)", width: "18px", height: "18px" }} /><Banknote size={20} color={formData.paymentMethod === "cod" ? "var(--primary-dark)" : "#666"} /><span style={{ fontWeight: formData.paymentMethod === "cod" ? "600" : "400" }}>Cash on Delivery</span></label>
                  </div>
                  {formData.paymentMethod === "cod" && <p style={{ marginTop: "8px", fontSize: "0.8rem", color: "#d97706" }}><ShieldCheck size={14}/> ₹100 COD standard shipping fee applies.</p>}
                </div>
                {apiError && <div style={{ color: "#d9534f", padding: "12px", background: "#ffebeb", borderRadius: "8px", marginBottom: "20px", fontSize: "0.9rem", textAlign: "center" }}>{apiError}</div>}
                <button type="submit" className="btn-primary w-100" disabled={isLoadingSecret} style={{ padding: "18px", fontSize: "1.05rem", marginTop: "10px" }}>{isLoadingSecret ? "Connecting Securely..." : "Complete Order • " + totalAmountStr}</button>
                <div style={{ textAlign: "center", marginTop: "16px", fontSize: "0.8rem", color: "#888" }}><ShieldCheck size={14} style={{ display: "inline", marginRight: "4px", verticalAlign: "middle" }} />Secure SSL Checkout via Stripe</div>
              </form>
            )}
          </div>
          <div className="cart-summary glass-panel" style={{ position: "static" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px", marginBottom: "24px", maxHeight: "300px", overflowY: "auto" }}>
              {cartItems.map(item => (
                <div key={item.id} style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                  <div style={{ position: "relative" }}><img src={item.image} alt={item.name} style={{ width: "60px", height: "60px", objectFit: "cover", borderRadius: "8px" }} /><span style={{ position: "absolute", top: "-8px", right: "-8px", background: "var(--primary-dark)", color: "white", fontSize: "0.7rem", width: "20px", height: "20px", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", fontWeight: "bold" }}>{item.quantity}</span></div>
                  <div style={{ flex: 1 }}><h4 style={{ fontSize: "0.9rem", margin: "0 0 4px", color: "#333" }}>{item.name}</h4><p style={{ margin: 0, fontSize: "0.85rem", color: "#666" }}>{item.price}</p></div>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginBottom: "20px" }}>
              <div style={{ display: "flex", gap: "8px" }}>
                <input type="text" placeholder="Promo code" value={promoCodeInput} onChange={(e) => setPromoCodeInput(e.target.value)} disabled={!!appliedPromo} style={{ flex: 1, padding: "10px", border: "1px solid #ddd", borderRadius: "6px" }} />
                {!appliedPromo ? <button type="button" onClick={handleApplyPromo} className="btn-secondary" style={{ padding: "10px 16px" }}>Apply</button> : <button type="button" onClick={handleRemovePromo} className="btn-outline-danger" style={{ padding: "10px 16px" }}>Remove</button>}
              </div>
